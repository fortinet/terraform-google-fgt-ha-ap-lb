...
run "img_select_by_version_short_byol" {
  ...
  assert {
    condition     = strcontains(google_compute_instance.fgt_vm[0].boot_disk[0].initialize_params[0].image, "727")
    error_message = "Image selected by firmware version should contain string '727' (assuming it's the newest one)"
  }
  assert {
    condition     = !strcontains(google_compute_instance.fgt_vm[0].boot_disk[0].initialize_params[0].image, "ondemand")
